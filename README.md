...
> **설치 커맨드** (`/dev-workflow:setup`)는 [Installation](#installation)을 참조하세요. 1.8.0부터 `.claude/rules/` 디렉토리 초기화 옵션이 추가되었습니다.
...
> ⚠️ 기존 `document-consolidation` Mode 3 (consolidate-domain)은 v1.9.0부터 deprecated, v2.0.0에서 제거 예정.
...
└── merge-to-domain.md      ← /dev-workflow:merge-to-domain → 관리자 도메인 머지 (v1.9.0+)
```
