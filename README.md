...
세션 시작 시 경량 부트스트랩(~2.3KB)만 주입되고, 워크플로우 의도 신호(단계 키워드, docs/design 작업, 재개 발화)가 있을 때만 오케스트레이터가 발동합니다 (v1.12.0). 발동 시 진행 중 작업을 복구하고, 단계를 감지한 뒤 필요한 경우에만 페르소나 확정·VCS 감지·Ouroboros 감지를 수행합니다. 일반 질문·단순 수정 세션에는 워크플로우 의식이 개입하지 않습니다. 진행 중 작업(HANDOFF)이 있으면 첫 메시지가 무엇이든 작업 목록이 제시됩니다.
...
> **설치 커맨드** (`/dev-workflow:setup`)는 [Installation](#installation)을 참조하세요. 1.8.0부터 `.claude/rules/` 디렉토리 초기화 옵션이 추가되었습니다. 1.9.0부터 결정 흐름 단계화가 도입되었고, 1.10.0에서 응답 주권 원칙·진행 모드 3종(하나씩/한꺼번에/알아서)·배포 환경 도달성 강화로 재설계되었습니다 (한 번에 하나씩 기본, 상호 순환 의존만 종합 질문, 충돌 시 재논의 대기열).
