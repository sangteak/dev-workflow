...
본 머지 스킬은 LLM 추론으로 SSOT를 변형한다. digest 추출·기계 검증·체크포인트는 MCP 도구가
결정적으로 수행하고 검증 페르소나가 위험을 통제하지만, 충돌 분류·문서 편집·의미 판정은
LLM 몫이라 의미 보존을 100% 보장하지 않는다. dry-run plan을 신중히 검토하고,
의심스러우면 abort하라.
...
본 스킬의 기계 단계는 **dev-workflow MCP 서버**의 도구 3종을 사용한다:

| 도구 | 담당 |
|---|---|
| `digest_extract` | (1)(2) 학습 — structured digest 결정적 추출 (멱등 ID 부여, REQ-005) |
| `merge_verify` | (5) 검증 — statement 정규화 매칭·라인 델타·변경 이력 행 기계 검사 |
| `merge_checkpoint` | (4) 적용 — no-git-mode 백업 생성/복원/목록 |

도구가 세션 도구 목록에 보이지 않으면 ToolSearch로 로드를 시도한다 (예: `select:` 정확명 또는 키워드 "digest merge checkpoint"). 그래도 부재하면 진입 흐름 1의 게이트에 따라 머지를 중단한다 — 자연어 절차로 대신 수행하지 않는다 (SSOT 저하 변형 금지).

**코드화 경계:** 충돌 분류·Architect 라운드·dry-run·사용자 승인·문서 편집과 의미 판단(immutability_level 부여, unmatched 패러프레이즈 판정, supersede 판정)은 자연어(LLM)에 남는다.
...
1. **Pre-flight**:
   - **도구 가용성 게이트**: dev-workflow MCP 서버의 `digest_extract`·`merge_verify`·`merge_checkpoint` 도구가 사용 가능한지 확인한다 — 목록에 없으면 ToolSearch로 로드를 시도하고, 그래도 부재하면 머지를 진행하지 않고 중단한다: "머지 도구(MCP)를 사용할 수 없습니다. Node.js 18+가 필요합니다 — `/dev-workflow:setup`으로 진단하세요."
   - **git-mode**: `git status`로 [해소된 경로] 하위 미커밋 변경을 확인한다 → 존재 시 경고하고 커밋/스태시를 유도한다. 사용자가 정리를 확인하기 전에는 (4) 적용 단계에 진입하지 않는다
...
(1) **domain.md 학습** — Structured Digest 추출 (`digest_extract` 도구)
(2) **feature.md 학습** — Structured Digest 추출 (`digest_extract` 도구)
(3) **머지 계획 수립** — 충돌 분류 + Architect 피드백 루프 (자연어)
(4) **적용** — dry-run → 사용자 승인 → domain.md 수정 (no-git 체크포인트: `merge_checkpoint` 도구)
(5) **검증** — `merge_verify` 도구 기계 검사 + Tech Lead/PM 체크리스트

**digest 사후 고지 노출 정책:** domain 학습 사후 고지는 대상 도메인 문서당 최초 1회만 노출한다. 누적 base 반영을 위한 재학습(두 번째 feature부터)은 내부 재추출(`digest_extract` 재호출)로 수행하고 고지를 생략한다 — 변경분은 직전 dry-run 승인에서 이미 검토됐다.

---

## (1) domain.md 학습

`digest_extract` 도구를 호출한다 — 입력 `{file_path: [domain.md 절대 경로], mode: "domain"}`.
출력(JSON: `policies`/`decisions`/`requirement_ids`/`section_order`/`line_count`)이 structured digest다.
자유 서술 요약은 **금지** — 도구 출력을 LLM이 재작성하지 않는다.

필드 정의와 후처리 규칙은 본 스킬의 references/templates.md 「digest 필드 정의와 LLM 후처리」를 Read하여 사용하라.

**Idempotent 의무 (REQ-005):**
- 멱등(동일 입력 → 동일 ID/순서)은 도구가 보장한다 — ID 부여 규칙(섹션 등장 순서 → 섹션 내 항목 순서 → 자동 번호, POL-001부터)과 명시 ID 보존 포함
- 재학습(도구 재호출) 시에도 동일한 ID/순서 산출

**LLM 후처리 (의미 필드):**
- 도구는 구조 신호(불릿·표·헤딩)만 사용하며 의미 판단 필드를 추출하지 않는다 — `immutability_level`·`supersedes` 등 의미 필드는 도구 출력에 LLM이 후처리로 부여한다 (규칙: templates.md 위 블록)

**누락 처리:**
- policies / decisions / requirement_ids가 빈 배열로 반환되면 그대로 둔다
- 호환성 첫 머지 체크리스트(아래 "호환성 첫 머지 체크리스트" 섹션 참조)에 따라 후속 처리

**대형 문서 (REQ-026):**
- 구 "80K 토큰 초과 시 섹션 단위 분할 학습" fallback은 도구 호출로 대체됐다 — 도구가 파일 전체를 메인 컨텍스트 소비 없이 처리하므로 분할이 불필요하다 (REQ 번호·이력은 보존)
...
(Read 실패 시 최소 요건: `📄 digest 추출 완료 — 정책 [N]건·결정 [M]건` 형식 1줄 고지 + 이의 제기 시 digest 노출·정정)

사용자가 이의를 제기하면 해당 digest를 즉시 노출하고 수정을 반영한다 — 도구 출력은 결정적이므로 정정은 LLM 후처리(분류·의미 필드)에서 반영하고, 원본 파일이 바뀐 경우에만 도구를 재호출한다 (안전망: development-principles "자동 결정 안전망" 참조)

---

## (2) feature.md 학습

각 feature.md에 대해 `digest_extract` 도구를 호출한다 — 입력 `{file_path: [feature.md 절대 경로], mode: "feature"}`.
`mode: "feature"`가 ID prefix `F-POL-`, `F-DEC-`를 부여한다 (domain과 구분).

**Idempotent 의무 (REQ-005):**
- domain.md 학습과 동일 — 멱등(동일 입력 → 동일 ID/순서, F-POL-001부터 자동 번호)은 도구가 보장한다
- 의미 필드 후처리도 domain과 동일하게 LLM이 수행한다 (templates.md 「digest 필드 정의와 LLM 후처리」)
...
사용자가 이의를 제기하면 해당 feature의 digest를 즉시 노출하고 수정을 반영한다 — 정정은 LLM 후처리에서 반영하고, 원본 파일이 바뀐 경우에만 도구를 재호출한다 (안전망: development-principles "자동 결정 안전망" 참조)
...
사용자 승인 후:
1. **no-git-mode 체크포인트**: domain.md 수정 직전에 `merge_checkpoint` 도구를 호출한다 — 입력 `{file_path: [도메인 문서 경로], action: "create"}` (같은 디렉토리에 `[도메인 문서명].premerge-YYYYMMDD(-N).bak` 생성. git-mode는 git이 복원 수단이므로 생략). 세션 정상 종료 시 체크포인트 파일을 정리한다
2. domain.md 수정
...
검증 첫 단계로 **`merge_verify` 도구를 호출한다** — 입력 `{pre_digest: (1)의 pre-merge digest(누적 base 기준), merged_file_path: [머지된 도메인 문서 경로], pre_line_count: pre-merge digest의 line_count}`.
도구는 기계 검사(정규화 문자열 매칭·라인 델타·변경 이력 행)까지만 수행한다 — 의미 판정은 LLM 몫.

| 항목 | 검증 방법 |
|---|---|
| 정책 보존 | merge_verify의 `matched`/`unmatched`. **unmatched 항목만 LLM이 의미 판정한다**: 패러프레이즈로 보존됐으면 매칭 인정, 아니면 정책 소실(실패 항목)로 처리. ID 집합 비교는 명시 ID 컬럼이 영구화된 문서에서만 사용한다 (위치 기반 자동 ID는 새 항목 삽입으로 밀릴 수 있어 단독 비교 기준으로 쓰지 않는다) |
| supersede 없는 정책 누락 X | (자연어) 정책 소실로 판정된 항목에 명시적 supersede가 있는지 확인 — supersede 판정은 LLM 몫 |
| 섹션 10 변경 이력 행 추가 | merge_verify의 `changelog_row_present`(오늘 날짜 행 존재) + 행 포맷 정확성은 LLM이 확인 |
| 라인 수 변화율 ±50% 이내 | merge_verify의 `line_delta_pct`/`line_delta_ok` (대량 손실 탐지) |
...
2. skip 또는 abort 선택 시 domain.md를 pre-merge 상태로 복원한다 — git-mode: `git checkout -- [도메인 문서]`, no-git-mode: `merge_checkpoint` 도구 호출 `{action: "restore"}` (가장 최근 체크포인트에서 복원)
...
- **Pass 1 (병렬)**: 카테고리별 서브에이전트가 (1)(2) 학습과 (3) 충돌 분류까지만 수행하고, digest(JSON)·머지 계획 초안·"사용자 결정 항목 목록"을 반환한다. 서브에이전트 프롬프트에 도구 사용 지시를 포함한다: "학습은 dev-workflow MCP 서버의 `digest_extract` 도구를 호출해 수행하라 (domain 문서는 `mode: "domain"`, feature 문서는 `mode: "feature"`). 도구가 목록에 없으면 ToolSearch로 로드를 시도하고, 그래도 부재하면 자연어로 추출하지 말고 도구 부재만 보고하라". 서브에이전트가 도구 부재를 보고하면 메인은 진입 흐름 1의 게이트와 동일하게 머지를 중단한다. 서브에이전트는 사용자에게 질문하거나 파일을 수정하지 않는다
- **digest 사후 고지 (메인, 직렬)**: 결정 해소 전, 메인 컨텍스트가 Pass 1이 반환한 domain digest를 **대상 도메인 문서당 1회** 사후 고지로 노출한다. feature digest는 카테고리당 1회 묶음으로 노출한다. 사용자가 이의를 제기하면 메인이 직접 정정한다 (도구 재호출 또는 LLM 후처리 수정)
...
| 플래그 | 의미 |
|---|---|
| `--no-review` | Architect 피드백 루프 강제 차단 (긴급 우회용, 사용자 책임) |
...
- 머지 세션 전체 중단
- 미커밋 상태의 도메인 문서 변경은 머지가 건드린 파일에 한정해 복원한다 — git-mode: `git checkout -- [도메인 문서]`, no-git-mode: `merge_checkpoint`(restore) 호출. **전체 `git reset`은 사용하지 않는다** (사용자의 다른 미커밋 작업 보호)
