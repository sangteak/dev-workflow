...
## 결정 흐름 상태
<!-- decision-flow.md §6에 따른 조건부 섹션 — 결정 흐름이 활성이 아니었으면 이 섹션 전체를 생략한다 -->
- 헤더: 확정 [N]/[M] · 진행 중: [결정명] · 재논의 [K]건
<!-- K는 아래 재논의 대기열 항목 수와 일치해야 한다 (불일치 시 대기열 항목 수로 재산출) -->
- ❓ 질문 진행: [N]/[M] · 국면: [국면명]
<!-- 질문 순회가 활성일 때만 포함 -->

### 결정 항목 (의존성 정렬 순서 보존 — 순서 자체가 상태다)
- [x] [항목명] — 확정: [선택 요지 한 줄]
- [>] [항목명] — 진행 중
- [ ] [항목명] — 미결
- [-] [항목명] — 무효화: [사유 한 줄]
...
<!-- 항목 전문을 기록한다. 건수만 쓰지 않는다 -->

### 미답변 질문
<!-- 질문 순회가 활성일 때만 포함 -->
- [질문 제목] — [현재 상태: 조사 대기 / 미노출]
...
해소된 경로 하위를 탐색하여 진행 중인 작업을 수집한 뒤, 아래 통합 목록 템플릿으로만 출력한다.
...
- 사용자 메시지의 의도가 목록의 **단일 항목과 명확히 일치**하면 목록 제시를 생략하고 해당 항목의 복구 시작 고지 1회로 축약한다 (예: 진행 중 작업이 1건뿐인데 "구현 이어서 하자" → 바로 "▶️ [기능명] DEVELOP부터 복구를 시작합니다.")
...
목록에서 항목을 선택하면 재확인 없이 복구를 시작하며 1줄 고지한다: "▶️ [기능명] — [국면/단계]부터 복구를 시작합니다 (HANDOFF 기반)" (선택 자체가 의사 표명 — 잘못 선택했으면 말씀하는 즉시 중단) (안전망: development-principles "자동 결정 안전망" 참조)

고지 후:
1. 완료된 phase 파일들을 로드한다
2. HANDOFF.md의 진행 내용을 기반으로 중단 지점부터 이어서 진행한다
3. HANDOFF에 "결정 흐름 상태" 섹션이 있으면: 한 줄 헤더(N/M/K)·미확정 잔여 순서·재논의 대기열을 원장 값으로 복원하고 `[>] 진행 중` 항목부터 결정 흐름을 재개한다. 대기열 내용을 기억으로 재구성하지 않는다 (decision-flow.md §6)
4. 국면 완료 → 해당 `phase*.md` 생성 → `HANDOFF.md` 삭제 (역할 완료)

### HANDOFF 없는 경우

목록에서 항목을 선택하면 재확인 없이 복구를 시작하며 1줄 고지한다: "▶️ [기능명] — [국면/단계]부터 복구를 시작합니다 (HANDOFF 없음 — 마지막 완료 단계 기준)" (선택 자체가 의사 표명 — 잘못 선택했으면 말씀하는 즉시 중단) (안전망: development-principles "자동 결정 안전망" 참조)
...
- `current-phase` 최신화
- 확정된 항목 / 미완료 항목 갱신
- 결정 흐름이 활성이면 "결정 흐름 상태" 섹션 갱신 (재논의 대기열은 항목 전문 기록 — decision-flow.md §6)
...
2. "HANDOFF.md 생성 규칙" 섹션의 템플릿에 따라 HANDOFF.md를 생성한다
   - 결정 흐름이 활성이면 "결정 흐름 상태" 섹션을 포함한다 (재논의 대기열은 항목 전문 기록)
...
1. 해소된 경로 하위를 탐색하여 진행 중인 작업을 수집한다 (탐색 절차 Step 1의 경로 해소 실행)
2. 통합 목록 템플릿으로 제시한다
