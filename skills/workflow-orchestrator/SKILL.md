...
4. **Persona Resolution** — **BRAINSTORM 또는 PLAN 판정 시에만** invoke `dev-workflow:persona-resolution`
   ...
   - DEVELOP/REVIEW/COMPLETION은 페르소나를 사용하지 않으므로 생략한다 (단 DEVELOP은 이슈 카드 관점 1줄 예외 — 「Issue Lifecycle」)
...
- 언어 최소 요건: 쉬운 한국어 · 전문 용어 즉석 풀이 · 줄임말 금지 · 문단 개행 (원본: decision-flow 3부 「언어 규범」 — Read 실패 시에도 이 줄이 적용된다)
