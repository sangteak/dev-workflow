...
1. 논의에 깔린 가정 중 검증되지 않은 것을 지적하라
2. 반사실적 질문(만약 ~라면?)을 1개 이상 던져라
3. 현재 방향의 대안을 1개 이상 제시하라
...
출력 재구성 (메인 컨텍스트):
```
[페르소나 이모지] [페르소나명] [Contrarian]: [비판/대안 내용]
```
결정 요청이 필요하면 메인 컨텍스트가 decision-flow.md 형식으로 재구성한다.
...
2. [대안] — [정통 해법 대비 이점 1줄]
```
제시 후 토론을 재개한다 — 대안 채택 여부는 기존 라운드 흐름으로 처리하고, 결정 요청은 decision-flow 형식을 따른다.
