...
### Step 2: MCP 실행 요건 진단 (비차단)

dev-workflow의 도메인 머지 도구(MCP 서버)는 **Node.js 18 이상**이 PATH에 있어야 기동된다.
Bash 도구로 확인한다:
...
결과에 따라 아래 한 블록만 표시하고 다음 Step으로 진행한다. 이 진단은 **비차단**이다 — 어떤 결과여도 설치 흐름을 멈추지 않는다:

**v18 이상:**

```
✅ Node.js [버전] — 도메인 머지 도구(MCP) 사용 가능
```

**명령을 찾을 수 없음 (미설치):**

```
⚠️ Node.js 미설치 — 도메인 머지(merge-to-domain) 기능만 비활성됩니다.
   나머지 워크플로우는 영향이 없습니다.
   설치: https://nodejs.org (LTS 권장) → 설치 후 Claude Code 재시작
```

**v18 미만:**

```
⚠️ Node.js [버전] — 도메인 머지 도구는 18 이상이 필요합니다.
   업그레이드: https://nodejs.org (LTS 권장) → 업그레이드 후 Claude Code 재시작
```
...
- 해당 플러그인의 마켓플레이스가 이미 `known_marketplaces.json`에 존재하면 → Step 5에서 `marketplace add` 명령을 **스킵**한다
- 존재하지 않으면 → Step 5에서 `marketplace add`를 실행한다
...
**4-C: installed_plugins.json 잔여 항목 정리**
...
1. 마켓플레이스 미등록 시 등록 명령 실행 (Step 4-A에서 이미 등록 확인됨 → 스킵)
2. 설치 명령을 실행 (Bash 도구)
...
2. **워크어라운드를 시도하지 않는다** — 수동 클론, 디렉토리 rename, git clone 등 어떤 대안적 방법도 시도하지 않는다. 사전 점검(Step 4)에서 처리하지 못한 문제는 사용자에게 맡긴다
3. **Step 6으로 즉시 넘어간다** — 성공한 플러그인은 검증하고, 실패한 플러그인은 수동 안내를 제공한다

이 규칙은 절대적이다. Step 4의 사전 점검이 알려진 문제를 미리 해결하므로, Step 5에서 실패하면 예상 밖의 문제이다. 이 경우 즉흥적 복구 시도는 상황을 악화시킬 수 있다.
