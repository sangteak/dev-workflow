...
이 도메인은 네 가지 축으로 구성된다. 첫째, `/compact` · phase 파일 · HANDOFF를 상황에 따라 전략적으로 사용하는 **컨텍스트 관리 3도구 전략**. 둘째, HANDOFF 저장과 복구를 slash command로 명시적으로 트리거하는 **명령 체계**. 셋째, 세션 시작 시 진행 중인 작업을 HANDOFF 유무와 관계없이 하나의 통합 목록으로 제시하는 **탐색 UX**. 넷째, 이슈 작업 상태의 **복구 규칙** — 신형 이슈 카드(v1.18.0)는 카드 자체가 상태라 HANDOFF가 필요 없고, 구형 이슈 디렉토리(레거시)에는 부모 Feature HANDOFF 오염을 막는 스코프 격리 규칙이 적용된다. 이 축들은 모두 `context-handling` 스킬을 SSOT(Single Source of Truth)로 삼아 동작하며, 정책이 분산되지 않도록 설계되어 있다.
...
HANDOFF가 없는 항목에는 `HANDOFF 없음` 태그를 부착한다. 이슈 항목은 부모 feature 하위에 들여쓰기로 표시한다 — 신형 이슈 카드(`issues/NNN-*.md`, v1.18.0)는 활성 카드(분석 절이 채워진 카드)만 들여쓰기로 표시해 선택 시 사이클을 재개하고, 대기 카드는 개수만 표시한다. 구형 이슈 디렉토리는 발견 시 "구 형식" 안내와 함께 표시한다. 정렬은 `last-updated` 역순이다. HANDOFF가 1개뿐일 때 자동 복귀하는 대안은 사용자 의도를 확인할 수 없고 새 작업 선택지를 보장할 수 없어 기각했다. 항상 목록을 제시하여 일관성을 유지한다.
...
## 이슈 서브워크플로우 HANDOFF 격리 (구형 전용)

> v1.18.0부터 이슈는 `issues/NNN-[문제명].md` 카드로 관리된다. 카드는 구조가 곧 상태(증상만=대기, 분석 채워짐=활성, 파일 없음=해소)라 HANDOFF를 만들지 않으며, 본 절의 격리 규칙은 구형 이슈 디렉토리(`issues/[문제명]/` — phase·HANDOFF 보유)에만 적용된다.
...
| 이슈 카드 복구 (v1.18.0) | 카드는 HANDOFF 불요 — 활성 카드만 목록 들여쓰기·선택 시 재개, 대기는 카운트, is-issue 규칙은 구형 전용 | 카드 구조가 상태를 인코딩 — 별도 보존 파일은 SSOT 이중화 |

## 변경 이력

| 날짜 | 변경 내용 | 영향 범위 | 상태 |
|------|-----------|-----------|------|
| 2026-07-13 | 기존 문서 — 신규 머지 시작 이전 | - | - |
| 2026-07-13 | issue-lifecycle 정합화 — 이슈 격리 절 구형 전용화, 신형 카드 수집·재개 규칙 반영 (작성자: Sangtaek Kwon (권상택)) | 시스템 개요, 세션 시작 탐색과 통합 목록, 이슈 서브워크플로우 HANDOFF 격리, 핵심 결정 사항, 변경 이력 신설 | 완료 |
