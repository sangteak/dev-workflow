...
- **REQ-013**: plan-stage 스킬이 Architect 페르소나 호출 시 `applies-to: plan` 매칭 규칙을 프롬프트에 첨부 — 우선순위: MEDIUM (`all` 규칙은 REQ-014에 따라 SessionStart에만 노출, 단계별 2차 첨부에서 제외)
