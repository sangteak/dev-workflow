...
affects:
  ...
  - plugin.json/marketplace.json (1.10.1 → 1.11.0 MINOR — 개발 시점 1.9.0에서 재조정)
---
...
- Cross-file 일관성 검증 통과 (개발 시점 버전 1.9.0 — 릴리스는 1.11.0으로 재조정, 스킬 카운트 11, Commands 6, 도메인 6)
...
| 2026-07-08 | 머지 전 리뷰(24건 확정) 반영 — Mode 1 삭제 범위 축소(후보 소멸 자기모순 해소), 대상 도메인 문서 해소·승격 흐름, no-git 체크포인트/복원, 병렬 2-pass 재정의, post-merge digest 검증, decision-flow SSOT 참조, pre-flight 가드. 릴리스 버전 1.9.0→1.11.0 재조정 (main이 1.9.0/1.10.x를 decision-flow에 선사용) | 본 문서 + merge-to-domain/document-consolidation 스킬 | 완료 |
